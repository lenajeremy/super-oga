...
// Same content stamp as the sprites: the URL changes when the photo does.
const stampOf = (file) => crypto.createHash('sha1').update(fs.readFileSync(file)).digest('hex').slice(0, 8);
const credits = PHOTOS.map(({ name, title, author, page, use }) => ({
  name,
  src: `assets/photos/${name}.jpg?v=${stampOf(path.join(OUT_DIR, `${name}.jpg`))}`,
  title, author, page, use, license: 'Pexels License',
}));
fs.writeFileSync(
