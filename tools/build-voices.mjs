...
for (const [id, [speaker, text]] of Object.entries(LINES)) {
  ...
  // Content stamp for the URL, so a re-recorded clip is a new URL and caches cleanly.
  // (`stamp` above is the cache key for deciding whether to re-record at all.)
  const version = crypto.createHash('sha1').update(fs.readFileSync(file)).digest('hex').slice(0, 8);
  manifest[id] = { src: `assets/voice/${id}.${ext}?v=${version}`, text, voice, speaker };
}
...
const kb = Object.values(manifest).reduce((n, m) => n + fs.statSync(path.join(root, m.src.split('?')[0])).size, 0) / 1024;
